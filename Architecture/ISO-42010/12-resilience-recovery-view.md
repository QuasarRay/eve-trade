...
stateDiagram-v2
  ...
  Unknown --> Reconcile: inspect idempotency, settlement metadata, Encore Pub/Sub, and logs
```
...
| Worker crashes before calling settlement | Command remains queued or is redelivered depending broker state. | Timeout | Encore Pub/Sub subscription handler recovery; monitor queue depth. |
...
| Inspect message headers and payload | SRE with settlement owner | Use NSQ HTTP inspection endpoint/CLI to inspect message ID, correlation ID, routing key, idempotency key, and payload class. | Procedure documented; tooling not verified |
...
| --- | --- | --- | --- | --- |
| RB-001 Ambiguous timeout reconciliation | Caller receives timeout or downstream unavailable after command submission. | Retry same command with same idempotency key; query `idempotency_record`; inspect `settlement_batch`, `request_attempt`, `settlement_step`; inspect Market/worker/settlement logs by idempotency key and settlement message ID. | On-call may perform read-only checks; manual mutation decisions are not automated. | Documented, not automated |
...
| Encore Pub/Sub command durability | Quorum queues are configured for command and dead-letter queues. | Broker backup/recovery and quorum sizing are not documented. |
| Service recovery | Kubernetes restarts pods; local Encore/Kubernetes restarts depend on local use. | No incident runbook or chaos test result recorded. |
...
5. If in progress beyond expected timeout, inspect Encore Pub/Sub queue state,
