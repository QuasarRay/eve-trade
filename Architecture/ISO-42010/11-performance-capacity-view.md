...
services, Encore Pub/Sub, and a PostgreSQL transaction. The architecture has timeout
configuration but no committed SLOs or measured capacity baseline.

```mermaid
flowchart LR
  Caller["Caller timeout not specified"] --> Gateway["gateway downstream timeout default 5s"]
  Gateway --> Market["Market settlement timeout default 10s"]
  Market --> Rabbit["Encore Pub/Sub publish timeout 5s"]
...
| --- | --- | --- | --- | --- |
| Caller to Encore gateway | No caller timeout is defined in the repository. | Unknown | No repository-level caller timeout contract. | Upstream may time out before internal settlement completes. |
| gateway to Market | `API_GATEWAY_DOWNSTREAM_TIMEOUT` default. | `5s` | Current default is shorter than Market settlement timeout. | Gateway may time out while Market continues. |
| Market to settlement transport | `Encore Pub/Sub publication and subscription retry settings` default. | `10s` | No approved end-to-end contract ties this to caller timeout. | Caller-visible latency includes broker, worker, settlement, and DB time. |
| Encore Pub/Sub publish | `Encore Pub/Sub publish behavior` configured in Kubernetes base. | `5s` | Publish/retry budget is not modeled end to end. | Publish confirmation failure can make outcome ambiguous if broker accepted the command but reply is lost. |
| settlement worker to trade-settlement | `SETTLEMENT_WORKER_REQUEST_TIMEOUT` configured in Kubernetes base. | `10s` | Coordination with asynchronous delivery is modeled at the subscription boundary. | Worker timeout can occur while database transaction outcome is unknown to the worker. |
...
| --- | --- | --- | --- |
| Encore gateway request latency SLO | Not measured | No p95/p99 latency is defined for issue, accept, cancel, or replay paths. | Gap recorded |
| Settlement end-to-end latency | Not measured | No budget from Market publish to settlement reply is recorded. | Gap recorded |
| Encore Pub/Sub queue depth threshold | Not measured | No warning/critical threshold for command queue or DLQ is recorded. | Gap recorded |
...
| --- | --- | --- | --- |
| Encore gateway request concurrency | HTTP latency and error rate | Kubernetes scaling and resource limits where configured. | No request concurrency target. |
...
| Encore Pub/Sub settlement worker subscription | Queue depth and publish latency | Quorum command queue and prefetch config. | No alert threshold or saturation policy. |
...
| Assumption | Status |
| --- | --- |
| Horizontal scaling of Encore gateway and Market is safe because they are stateless with respect to business mutation. | Reasonable, but load-tested capacity is not documented. |
...
| Encore Pub/Sub quorum queues improve durability at a latency cost. | Accepted architecture trade-off; no benchmark recorded. |
...
| --- | --- | --- |
| The architecture has configurable timeouts across Encore gateway, Market, messaging, and worker. | Enforced by configuration | Config files define defaults and Kubernetes values. |
