...
| FACT-031 | Market exposes `/healthz` and `/readyz`; current readiness checks PostgreSQL through repository readiness only. | Evidence-backed with gap | `distributed-backend/src/market/distributed-backend/server.go`; `06-deployment-operations-view.md` |
...
| FACT-050 | Settlement metadata tables are the durable diagnostic source for idempotency, attempts, batches, and steps. | Evidence-backed | `distributed-backend/src/trade-settlement/migrations/0001_settlement_schema.sql` |
...
| FACT-080 | Runtime validation was not run for this documentation-only update. | Not run | `08-development-validation-view.md`, `18-evidence-manifest.md` |
