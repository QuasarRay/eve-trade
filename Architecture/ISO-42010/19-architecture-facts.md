...
| FACT-007 | The Django simulator is a local game-frontend simulator and its outbound UDP packet conforms to the versioned repository schema and a golden packet consumed by both Python and Go tests. No claim of identity with an external game client is made. | Evidence-backed | `distributed-backend/protocol`, `simulator/trade_gui/tests.py`, `distributed-backend/src/api-gateway/distributed-backend/quilkin_udp_test.go` |
