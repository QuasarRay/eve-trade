...
| --- | --- | --- |
| UDP edge exposure | Quilkin is the UDP entry point; Encore gateway validates packet size, empty packets, HMAC signature, replay, rate limits, queue capacity, and downstream timeout before forwarding. | Edge replay cache is process-local; external DDoS controls are outside this repo. |
...
| Settlement operation privilege | In the checked-in production-like topology, trade-settlement receives protovalidate-checked requests from settlement worker. | The settlement protobuf exposes generic mutation operations; no operation-provenance or operation-allow policy is implemented in trade-settlement. |
