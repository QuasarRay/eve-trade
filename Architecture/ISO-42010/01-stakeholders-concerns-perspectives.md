...
| STK-03 | Backend service developer | Evolves Go and Rust services, Encore API types and the remaining Rust protobuf contract, messaging, and SQL-backed behavior. |
...
- CON-12: Service readiness should reflect downstream dependency availability
  where implemented.
- CON-13: Encore Pub/Sub subscription processing must separate normal messages from dead
  messages.
...
| --- | --- | --- | --- |
| ASP-01 | Contract compatibility | CON-01, CON-03, CON-04, CON-28 | Public and internal API messages are Encore API types and the remaining Rust protobuf contract shared by Encore gateway and Market, with settlement contracts shared by Market, messaging, worker, and trade-settlement. |
...
| ASP-04 | Asynchronous settlement | CON-12, CON-13, CON-14, CON-15 | Market publishes typed settlement work to Encore Pub/Sub; the settlementworker service invokes Rust trade-settlement. |
| ASP-05 | Trust boundary control | CON-17, CON-18, CON-19, CON-20, CON-22 | Encore gateway is game-facing; Market owns trade policy; settlement is internal in the checked-in production-like topology and executes requested operation batches. |
| ASP-06 | Deployment parity | CON-16, CON-23, CON-24, CON-26 | Local local Encore/Kubernetes and Kubernetes both model the same service chain and database/messaging dependencies. |
...
| CON-12 | High | Stakeholder need | Health handlers, local Encore/Kubernetes, Kubernetes probes | E2 symbol/object | STK-05, STK-09 | Gap recorded |
| CON-13 | High | Stakeholder need | Encore Pub/Sub topic and subscription topology and worker behavior | E2 symbol/object | STK-05, STK-09 | Unvalidated; operations runbook needed |
...
| CON-16 | High | Architectural constraint | local Encore/Kubernetes and Kubernetes manifests | E2 symbol/object | STK-05, STK-10 | Unvalidated |
| CON-17 | Critical | Architectural constraint | Encore gateway boundary and network policy | E2 symbol/object | STK-06 | Gap recorded |
...
| CON-21 | High | Architectural constraint | Kubernetes Secrets and local Encore/Kubernetes env vars | E2 symbol/object | STK-05, STK-06 | Gap recorded |
| CON-22 | High | Architectural constraint | Istio strict mTLS and local h2c | E2 symbol/object | STK-05, STK-06 | Gap recorded |
| CON-23 | High | Stakeholder need | local Encore/Kubernetes and Kubernetes manifests | E2 symbol/object | STK-05 | Unvalidated |
| CON-24 | Critical | Architectural constraint | Migration job and local Encore/Kubernetes migrate service | E2 symbol/object | STK-07, STK-05 | Unvalidated |
...
| --- | --- | --- | --- |
| Asynchronous settlement path over Encore Pub/Sub | CON-14, CON-15, CON-13 | Market returns queued settlement metadata after publishing work; final durable outcome is recovered through idempotency/status data. | Timeout-after-commit outcomes require idempotency-based recovery. |
...
| Local convenience versus production isolation | CON-16, CON-18, CON-21, CON-22 | local Encore/Kubernetes exposes PostgreSQL and Encore Pub/Sub on loopback; production uses network policy and mesh controls. | Local exposure must not be copied to production. |
...
| Deployment portability versus platform-specific controls | CON-23, CON-27 | local Encore/Kubernetes and Kubernetes share logical topology; Terraform has AWS/EKS, GCP/GKE, and Talos/Omni roots. | Some behavior depends on provider-specific networking, registry, database, secrets, storage, and ingress behavior. |
...
| ASP-04 Asynchronous settlement | Functional/Runtime, Performance/Capacity, Resilience/Recovery | VC-RUN-01, VC-RUN-03, VC-PERF-01, VC-RES-01 | Encore Pub/Sub topic and subscription topology and failure semantics are explicit; DLQ runbook remains a risk. |
...
| ASP-06 Deployment parity | Deployment/Operations, Development/Validation | VC-DEP-01, VC-VAL-01 | local Encore/Kubernetes versus Kubernetes differences are listed explicitly. |
