# This block makes all filesystem and command failures terminate the script immediately.
# It exists because partially-written integration files are worse than no files at all.
$ErrorActionPreference = "Stop"

# This block locates the repository root from the place where this script is executed.
# It exists so the script can be copied to the repo root and run without extra arguments.
$RepoRoot = Resolve-Path "."

# This block creates the directories needed by Docker images, scripts, and e2e tests.
# It exists so later file writes do not depend on folders already existing.
New-Item -ItemType Directory -Force -Path "distributed-backend/docker" | Out-Null
New-Item -ItemType Directory -Force -Path "distributed-backend/scripts" | Out-Null
New-Item -ItemType Directory -Force -Path "distributed-backend/tests/e2e" | Out-Null

# This block writes the Docker Compose integration harness.
# It exists so market,  settlement, PostgreSQL, migrations, and e2e tests run as one deterministic stack.
@'
# This block names the compose project so Docker resources are grouped under one predictable prefix.
# It exists so repeated integration runs are easy to inspect and clean up with `docker compose down -v`.
name: eve-trade-integration

# This block declares the  runtime services used by the integration harness.
# It exists because these tests intentionally exercise market -> trade-settlement -> PostgreSQL instead of replacing settlement with a fake.
services:
  # This block starts the only durable storage dependency required by the MVP trade path.
  # It exists so settlement writes against a  PostgreSQL engine,  enum types,  constraints, and  transactions.
  postgres:
    image: postgres:16
    environment:
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: postgres
      POSTGRES_DB: eve_trade
    ports:
      - "5432:5432"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres -d eve_trade"]
      interval: 2s
      timeout: 2s
      retries: 30
    volumes:
      - eve_trade_pgdata:/var/lib/postgresql/data

  # This block applies the canonical SQL migrations before either service accepts traffic.
  # It exists so the test database is recreated deterministically from the same migration files the project owns.
  migrate:
    image: postgres:16
    depends_on:
      postgres:
    ...
    command: >
      bash -lc "
      psql -h postgres -U postgres -d eve_trade -v ON_ERROR_STOP=1 -c 'DROP SCHEMA IF EXISTS trade CASCADE; DROP EXTENSION IF EXISTS pgcrypto;' &&
      psql -h postgres -U postgres -d eve_trade -v ON_ERROR_STOP=1 -f distributed-backend/migrations/postgresql/0001_create_trade_schema.up.sql &&
      psql -h postgres -U postgres -d eve_trade -v ON_ERROR_STOP=1 -f distributed-backend/migrations/postgresql/0002_make_checksum_algorithm_match_code.sql
      "

  # This block builds and runs the  Rust settlement service from the current repository checkout.
  # It exists because market must talk to the  correctness-critical service over gRPC-compatible transport.
  trade-settlement:
    build:
      context: .
      dockerfile: distributed-backend/docker/trade-settlement.Dockerfile
    depends_on:
      migrate:
        condition: service_completed_successfully
    environment:
      DATABASE_URL: postgres://postgres:postgres@postgres:5432/eve_trade
    ports:
      - "9092:9092"

  # This block builds and runs the  Go market service from the current repository checkout.
  # It exists so the tests cover market validation/translation plus  settlement calls, not settlement alone.
  market:
    build:
      context: .
      dockerfile: distributed-backend/docker/market.Dockerfile
    depends_on:
      trade-settlement:
        condition: service_started
    environment:
      MARKET_ADDR: :8081
      SETTLEMENT_URL: http://trade-settlement:9092
    ports:
      - "8081:8081"

  # This block runs the Go e2e test package inside the same Docker network as market, settlement, and PostgreSQL.
  # It exists so CI and local runs use the same network names and do not depend on host-specific service discovery.
  e2e-tests:
    ...
    environment:
      EVE_TRADE_MARKET_URL: http://market:8081
      EVE_TRADE_DATABASE_URL: postgres://postgres:postgres@postgres:5432/eve_trade
    command: >
      bash -lc "go test -tags=e2e ./distributed-backend/tests/e2e -count=1 -v"

# This block stores PostgreSQL state between container restarts while still allowing full cleanup via `down -v`.
# It exists so local runs are fast, but deterministic reset is still handled by the migrate service.
volumes:
  eve_trade_pgdata:
  go_cache:
  go_build_cache:

'@ | Set-Content -NoNewline -Encoding UTF8 "docker-compose.integration.yml"

# This block writes the trade-settlement Dockerfile.
# It exists so Compose can build and run the  Rust settlement service from the current checkout.
@'
...
'@ | Set-Content -NoNewline -Encoding UTF8 "distributed-backend/docker/trade-settlement.Dockerfile"

# This block writes the market Dockerfile.
# It exists so Compose can build and run the  Go market service from the current checkout.
@'
...
'@ | Set-Content -NoNewline -Encoding UTF8 "distributed-backend/docker/market.Dockerfile"

# This block writes the PowerShell test runner.
# It exists so Windows local development has one deterministic command for the -service integration suite.
@'
...
'@ | Set-Content -NoNewline -Encoding UTF8 "distributed-backend/scripts/run-e2e.ps1"

# This block writes the Go e2e test file.
# It exists so tests call  market,  settlement, and  PostgreSQL instead of a fake settlement implementation.
@'
//go:build e2e
...
'@ | Set-Content -NoNewline -Encoding UTF8 "distributed-backend/tests/e2e/market_settlement_test.go"

# This block adds the PostgreSQL driver dependency used by the e2e tests and refreshes module metadata.
# It exists because the existing Go module only needed connect/protobuf before database-backed assertions.
go get github.com/jackc/pgx/v5@v5.7.6
go mod tidy

# This block formats the generated Go test file.
# It exists so the new tests respect the same formatting standard as the existing CI workflow.
gofmt -w "distributed-backend/tests/e2e/market_settlement_test.go"

# This block prints the next command to run without starting Docker automatically.
# It exists so you can inspect the generated files before executing the integration stack.
Write-Host "Generated -service integration harness. Run: .\distributed-backend\scripts\run-e2e.ps1"
