# Scenario contract: `market_to_pubsub_directional_partition`
...
- Workload actors/input schema: `durably accepted market operation` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `encore-backend egress to nsqd`
...
- Affected path/resource: `market-to-nsqd`
- Activation barrier: SQL witness observes the durable operation/outbox row before activation.
- Litmus definition/reference: `pod-network-partition` against `encore-backend egress to nsqd` on `market-to-nsqd`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent source-to-destination network probe failure are required.
...
- Release/recovery barrier: partition is absent, probe succeeds, and durable operation reaches broker.
...
- `recovery.condition` — partition is absent, probe succeeds, and durable operation reaches broker.
...
- `chaos.target` — Litmus targets exactly encore-backend egress to nsqd and the affected path is market-to-nsqd.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent source-to-destination network probe failure are required.
- `chaos.activation` — SQL witness observes the durable operation/outbox row before activation.
