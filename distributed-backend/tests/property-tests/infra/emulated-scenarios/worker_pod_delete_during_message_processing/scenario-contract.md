# Scenario contract: `worker_pod_delete_during_message_processing`
...
- Exact target: `encore-backend pod hosting settlement-worker, selected by exact labels and UID`
...
- Affected path/resource: `nsqd-to-worker-to-settlement`
- Activation barrier: application/SQL witness observes the message processing owner.
- Litmus definition/reference: `pod-delete` against `encore-backend pod hosting settlement-worker, selected by exact labels and UID` on `nsqd-to-worker-to-settlement`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement, Kubernetes UID loss, and broker in-flight transition prove the effect.
...
- Release/recovery barrier: replacement worker is ready and the message is no longer permanently in flight.
...
- `recovery.condition` — replacement worker is ready and the message is no longer permanently in flight.
...
- `chaos.target` — Litmus targets exactly encore-backend pod hosting settlement-worker, selected by exact labels and UID and the affected path is nsqd-to-worker-to-settlement.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement, Kubernetes UID loss, and broker in-flight transition prove the effect.
- `chaos.activation` — application/SQL witness observes the message processing owner.
