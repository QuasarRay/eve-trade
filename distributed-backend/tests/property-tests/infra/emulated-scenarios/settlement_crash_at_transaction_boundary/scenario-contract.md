# Scenario contract: `settlement_crash_at_transaction_boundary`
...
- Workload actors/input schema: `settlement request and status recovery workload` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `trade-settlement container`
- Runtime-enforced Kubernetes target policy: `{"kind":"deployment","selector":"app.kubernetes.io/name=trade-settlement"}`.
- Runtime-enforced fixed Litmus environment: `{"TARGET_CONTAINER":"trade-settlement"}`.
- Affected path/resource: `worker-to-settlement-to-postgres`
- Activation barrier: production-disabled settlement failpoint confirms the named transaction boundary.
- Litmus definition/reference: `container-kill` against `trade-settlement container` on `worker-to-settlement-to-postgres`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Kubernetes observes container termination after the confirmed boundary.
...
- Release/recovery barrier: settlement is ready and idempotency-key status is independently queryable.
...
- `recovery.condition` — settlement is ready and idempotency-key status is independently queryable.
...
- `chaos.target` — Litmus targets exactly trade-settlement container and the affected path is worker-to-settlement-to-postgres.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Kubernetes observes container termination after the confirmed boundary.
- `chaos.activation` — production-disabled settlement failpoint confirms the named transaction boundary.
...
Trade-settlement has no production-disabled transaction-boundary failpoints.
