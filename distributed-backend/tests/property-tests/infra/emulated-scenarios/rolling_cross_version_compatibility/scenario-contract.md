# Scenario contract: `rolling_cross_version_compatibility`
...
- External dependencies: encore-backend, trade-settlement, settlement-worker, postgres, nsqd.
- Workload actors/input schema: `cross-version contract workload` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_ROLLING_VERSION_CHANGE`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_ROLLING_VERSION_CHANGE`
- Exact target: `selected application deployment`

- Affected path/resource: `versioned producer-consumer path`
- Activation barrier: both immutable image digests are ready in the same namespace.
...
- Independent effect witness: Kubernetes pod image IDs and application build-info endpoints identify both versions.
...
- Release/recovery barrier: rollout or rollback reaches its declared replica set.
...
- `recovery.condition` — rollout or rollback reaches its declared replica set.
...
No repository-authoritative immediately-previous image digest is pinned for this checkout.
