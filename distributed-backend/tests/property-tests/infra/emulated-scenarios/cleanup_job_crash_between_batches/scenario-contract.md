# Scenario contract: `cleanup_job_crash_between_batches`
...
- External dependencies: postgres, settlement-worker.
- Workload actors/input schema: `multiple identified retention-eligible batches plus protected control rows` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `APPLICATION_CLEANUP_BATCH_FAILPOINT`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `APPLICATION_CLEANUP_BATCH_FAILPOINT`
- Exact target: `retention cleanup worker between committed batches`

- Affected path/resource: `cleanup-worker-to-postgres`
- Activation barrier: the first exact cleanup batch is committed and the next eligible row set is observed.
...
- Independent effect witness: application and SQL observations prove the cleanup process stopped at the inter-batch boundary.
...
- Release/recovery barrier: the cleanup worker restarts, resumes, and leaves protected control rows unchanged.
...
- `recovery.condition` — the cleanup worker restarts, resumes, and leaves protected control rows unchanged.
...
The local deployment has no independently addressable cleanup worker or production-disabled inter-batch crash boundary.
