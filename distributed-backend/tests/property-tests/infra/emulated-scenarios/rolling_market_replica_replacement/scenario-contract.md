# Scenario contract: `rolling_market_replica_replacement`
...
- External dependencies: encore-backend, postgres, nsqd.
...
- Activation barrier: durable operation identity exists before old replica termination.
...
- Independent effect witness: pod UID transition and SQL/outbox observations bind the operation across the rollout.
...
- Release/recovery barrier: rollout completes and operation reaches terminal publication state.
...
- `recovery.condition` — rollout completes and operation reaches terminal publication state.
