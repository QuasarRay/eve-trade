# Scenario contract: `random_crash_restart_sequence`
...
- Workload actors/input schema: `identified trade workload spanning seeded crash steps` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_SEEDED_POD_DELETE_SEQUENCE`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_SEEDED_POD_DELETE_SEQUENCE`
- Exact target: `seed-selected pods from the explicit scenario allowlist`
- Runtime-enforced Kubernetes target policy: `"${ANYSYSTEM_SELECTED_VARIANT}"`.
- Runtime-enforced fixed Litmus environment: `{}`.
- Affected path/resource: `seed-selected application path`
- Activation barrier: controller trace records selected target and ready-state barrier before each deletion.
- Litmus definition/reference: `pod-delete` against `seed-selected pods from the explicit scenario allowlist` on `seed-selected application path`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: each Litmus acknowledgement is paired with Kubernetes UID disappearance/replacement evidence.
...
- Release/recovery barrier: every selected component passes its declared recovery barrier before the next logical tick.
...
- `recovery.condition` — every selected component passes its declared recovery barrier before the next logical tick.
...
- `chaos.target` — Litmus targets exactly seed-selected pods from the explicit scenario allowlist and the affected path is seed-selected application path.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — each Litmus acknowledgement is paired with Kubernetes UID disappearance/replacement evidence.
- `chaos.activation` — controller trace records selected target and ready-state barrier before each deletion.
