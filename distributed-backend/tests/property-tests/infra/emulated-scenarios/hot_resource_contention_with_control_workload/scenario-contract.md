# Scenario contract: `hot_resource_contention_with_control_workload`
...
- Workload actors/input schema: `hot-resource workload plus unrelated identified control workload` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_CONCURRENT_WORKLOAD`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_CONCURRENT_WORKLOAD`
- Exact target: `scenario-selected hot trade, wallet, or outbox stream`

- Affected path/resource: `application-to-shared-resource and unrelated control path`
- Activation barrier: hot workload reaches declared in-flight/lock/backlog threshold.
...
- Independent effect witness: independent SQL/broker probe observes hot pressure while control identities remain distinct.
...
- Release/recovery barrier: both workloads complete and pressure metrics return to baseline.
...
- `recovery.condition` — both workloads complete and pressure metrics return to baseline.
