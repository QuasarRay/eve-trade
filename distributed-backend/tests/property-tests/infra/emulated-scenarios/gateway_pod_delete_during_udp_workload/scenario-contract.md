# Scenario contract: `gateway_pod_delete_during_udp_workload`
...
- Workload actors/input schema: `authenticated UDP issue/accept/cancel requests` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `encore-backend pod selected by exact labels and UID`
...
- Activation barrier: workload request is observed at the selected gateway before deletion.
- Litmus definition/reference: `pod-delete` against `encore-backend pod selected by exact labels and UID` on `quilkin-to-gateway`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and Kubernetes old-pod UID disappearance/replacement are both required.
...
- Release/recovery barrier: replacement pod is ready and UDP control probe succeeds.
...
- `recovery.condition` — replacement pod is ready and UDP control probe succeeds.
...
- `chaos.target` — Litmus targets exactly encore-backend pod selected by exact labels and UID and the affected path is quilkin-to-gateway.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and Kubernetes old-pod UID disappearance/replacement are both required.
- `chaos.activation` — workload request is observed at the selected gateway before deletion.
