# Scenario contract: `poison_message_retry_and_quarantine`
...
- External dependencies: nsqd, settlement-worker, postgres.
- Workload actors/input schema: `poison message followed by healthy identified work` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `BROKER_PUBLISH_POISON_AND_CONTROL`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `BROKER_PUBLISH_POISON_AND_CONTROL`
- Exact target: `declared correctness-critical NSQ channel`

- Affected path/resource: `nsqd-to-settlement-worker`
- Activation barrier: poison payload and healthy control payload are durably published in order.
...
- Independent effect witness: broker counters and application logs identify poison redeliveries and later healthy-message processing.
...
- Release/recovery barrier: poison message reaches declared terminal handling and healthy work completes.
...
- `recovery.condition` — poison message reaches declared terminal handling and healthy work completes.
