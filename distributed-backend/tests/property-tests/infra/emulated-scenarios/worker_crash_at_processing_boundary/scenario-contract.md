# Scenario contract: `worker_crash_at_processing_boundary`
...
- Workload actors/input schema: `settlement work message and idempotent recovery workload` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `encore-backend container hosting the settlement-worker service`
...
- Affected path/resource: `nsqd-to-worker-to-trade-settlement`
- Activation barrier: production-disabled worker failpoint confirms the named boundary.
- Litmus definition/reference: `container-kill` against `encore-backend container hosting the settlement-worker service` on `nsqd-to-worker-to-trade-settlement`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Kubernetes observes the selected container terminate and message remains independently visible.
...
- Release/recovery barrier: another worker owns or completes the operation and the channel drains.
...
- `recovery.condition` — another worker owns or completes the operation and the channel drains.
...
- `chaos.target` — Litmus targets exactly encore-backend container hosting the settlement-worker service and the affected path is nsqd-to-worker-to-trade-settlement.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Kubernetes observes the selected container terminate and message remains independently visible.
- `chaos.activation` — production-disabled worker failpoint confirms the named boundary.
...
The worker has no exact boundary failpoints for receive/lease/gRPC/publish/ack phases.
