# Scenario contract: `gateway_cpu_pressure`
...
- Workload actors/input schema: `authenticated duplicate/replay workload plus unaffected-replica control` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_CPU_HOG`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_CPU_HOG`
- Exact target: `selected encore-backend pod`
...
- Affected path/resource: `quilkin-to-selected-gateway`
- Activation barrier: selected and control replica identities plus CPU baseline are recorded.
- Litmus definition/reference: `pod-cpu-hog` against `selected encore-backend pod` on `quilkin-to-selected-gateway`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent container CPU metric cross the declared threshold.
...
- Release/recovery barrier: CPU stress is absent and selected replica returns to readiness.
...
- `recovery.condition` — CPU stress is absent and selected replica returns to readiness.
...
- `chaos.target` — Litmus targets exactly selected encore-backend pod and the affected path is quilkin-to-selected-gateway.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent container CPU metric cross the declared threshold.
- `chaos.activation` — selected and control replica identities plus CPU baseline are recorded.
