# Scenario contract: `concurrent_item_reservation_race`
...
- External dependencies: settlement-worker, trade-settlement, postgres.
- Workload actors/input schema: `multiple issue requests sharing one item stack` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `source item-stack row`

- Affected path/resource: `concurrent item-reservation transactions`
- Activation barrier: all contenders report arrival before the same stack mutation boundary.
...
- Independent effect witness: independent PostgreSQL lock/activity probe identifies every contender.
...
- Release/recovery barrier: all contenders finish and item row locks are released.
...
- `recovery.condition` — all contenders finish and item row locks are released.
...
The real settlement path has no stack pre-mutation race barrier.
