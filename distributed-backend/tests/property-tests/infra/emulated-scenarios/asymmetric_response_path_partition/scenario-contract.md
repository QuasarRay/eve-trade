# Scenario contract: `asymmetric_response_path_partition`
...
- Workload actors/input schema: `identified idempotent request and replay` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `scenario-selected response path`
- Runtime-enforced Kubernetes target policy: `{"kind":"deployment","selector":"app.kubernetes.io/name=encore-backend"}`.
- Runtime-enforced fixed Litmus environment: `{"NAMESPACE_SELECTOR":"kubernetes.io/metadata.name=${RUN_NAMESPACE}","POD_SELECTOR":"app.kubernetes.io/name=nsqd","POLICY_TYPES":"egress"}`.
- Affected path/resource: `declared request/response service edge`
- Activation barrier: request identity is observed at the destination before response-path activation.
- Litmus definition/reference: `pod-network-partition` against `scenario-selected response path` on `declared request/response service edge`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement plus directional probes prove request success and response failure.
...
- Release/recovery barrier: both directional probes succeed and status/replay path is available.
...
- `recovery.condition` — both directional probes succeed and status/replay path is available.
...
- `chaos.target` — Litmus targets exactly scenario-selected response path and the affected path is declared request/response service edge.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement plus directional probes prove request success and response failure.
- `chaos.activation` — request identity is observed at the destination before response-path activation.
