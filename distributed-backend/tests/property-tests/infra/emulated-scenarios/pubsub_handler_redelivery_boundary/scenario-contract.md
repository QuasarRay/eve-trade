# Scenario contract: `pubsub_handler_redelivery_boundary`
...
- External dependencies: nsqd, settlement-worker, trade-settlement, postgres.
- Workload actors/input schema: `single identified NSQ message and handler workload` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `WORKER_HANDLER_FAILPOINT_AND_BROKER_REDELIVERY`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `WORKER_HANDLER_FAILPOINT_AND_BROKER_REDELIVERY`
- Exact target: `settlement work channel handler`

- Affected path/resource: `nsqd-to-settlement-worker`
- Activation barrier: handler failpoint confirms the exact commit/publish/ack boundary.
...
- Independent effect witness: NSQ stats independently show the message in-flight or requeued with the same identity.
...
- Release/recovery barrier: message is terminally acknowledged or quarantined and the channel is healthy.
...
- `recovery.condition` — message is terminally acknowledged or quarantined and the channel is healthy.
...
The worker has no exact handler-boundary failpoints and NSQ test hooks for deterministic redelivery timing.
