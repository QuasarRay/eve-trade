# Scenario contract: `gateway_memory_pressure`
...
- Workload actors/input schema: `authenticated overload workload` with exact request, actor, entity, scenario, seed, and action identities.
...
- Activation barrier: selected pod identity and memory baseline are recorded.
- Litmus definition/reference: `pod-memory-hog` against `selected encore-backend pod` on `quilkin-to-selected-gateway`.
...
- Release/recovery barrier: memory stress is absent and gateway is ready.
...
- `recovery.condition` — memory stress is absent and gateway is ready.
...
- `chaos.activation` — selected pod identity and memory baseline are recorded.
