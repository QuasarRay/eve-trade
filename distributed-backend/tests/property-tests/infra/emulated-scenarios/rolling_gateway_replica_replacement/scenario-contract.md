# Scenario contract: `rolling_gateway_replica_replacement`
...
- Workload actors/input schema: `authenticated UDP requests with stable idempotency identities` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_ROLLOUT_RESTART`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_ROLLOUT_RESTART`
- Exact target: `encore-backend deployment`

- Affected path/resource: `quilkin-to-gateway`
- Activation barrier: old and new replica identities overlap at the declared rollout phase.
...
- Independent effect witness: Kubernetes EndpointSlice and pod UID observations prove unready routing removal and replacement.
...
- Release/recovery barrier: rollout completes with required ready replicas.
...
- `recovery.condition` — rollout completes with required ready replicas.
