# Scenario contract: `worker_memory_pressure`
...
- Workload actors/input schema: `identified settlement work and healthy control message` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_MEMORY_HOG`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_MEMORY_HOG`
- Exact target: `selected encore-backend pod hosting settlement-worker`
...
- Affected path/resource: `nsqd-to-selected-worker`
- Activation barrier: selected pod identity, broker baseline, and memory baseline are recorded.
- Litmus definition/reference: `pod-memory-hog` against `selected encore-backend pod hosting settlement-worker` on `nsqd-to-selected-worker`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent working-set probe observe declared pressure.
...
- Release/recovery barrier: memory stress is absent, worker is ready, and unfinished messages are recoverable.
...
- `recovery.condition` — memory stress is absent, worker is ready, and unfinished messages are recoverable.
...
- `chaos.target` — Litmus targets exactly selected encore-backend pod hosting settlement-worker and the affected path is nsqd-to-selected-worker.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent working-set probe observe declared pressure.
- `chaos.activation` — selected pod identity, broker baseline, and memory baseline are recorded.
