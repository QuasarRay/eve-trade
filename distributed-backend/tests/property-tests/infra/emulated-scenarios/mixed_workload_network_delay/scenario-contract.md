# Scenario contract: `mixed_workload_network_delay`
...
3. `activate-scenario-chaos` — `LITMUS_NETWORK_LATENCY`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_NETWORK_LATENCY`
- Exact target: `scenario-selected application path`
...
- Affected path/resource: `declared source-to-destination service edge`
- Activation barrier: clean baseline latency distribution is recorded.
- Litmus definition/reference: `pod-network-latency` against `scenario-selected application path` on `declared source-to-destination service edge`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent probe observes latency above the generated lower bound.
...
- Release/recovery barrier: latency qdisc is absent and recovered probe returns within the bounded control envelope.
...
- `recovery.condition` — latency qdisc is absent and recovered probe returns within the bounded control envelope.
...
- `chaos.target` — Litmus targets exactly scenario-selected application path and the affected path is declared source-to-destination service edge.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent probe observes latency above the generated lower bound.
- `chaos.activation` — clean baseline latency distribution is recorded.
