# Scenario contract: `quilkin_gateway_packet_duplication`
...
- Workload actors/input schema: `sequence-numbered authenticated UDP requests` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_NETWORK_DUPLICATION`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_NETWORK_DUPLICATION`
- Exact target: `quilkin egress to encore-backend UDP`
- Runtime-enforced Kubernetes target policy: `{"kind":"deployment","selector":"app.kubernetes.io/name=quilkin"}`.
- Runtime-enforced fixed Litmus environment: `{}`.
- Affected path/resource: `quilkin-to-gateway`
- Activation barrier: clean sequence-numbered UDP control exchange is observed.
- Litmus definition/reference: `pod-network-duplication` against `quilkin egress to encore-backend UDP` on `quilkin-to-gateway`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent UDP sequence probe observes duplicate datagrams.
...
- Release/recovery barrier: duplication qdisc is absent and clean control exchange succeeds.
...
- `recovery.condition` — duplication qdisc is absent and clean control exchange succeeds.
...
- `chaos.target` — Litmus targets exactly quilkin egress to encore-backend UDP and the affected path is quilkin-to-gateway.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent UDP sequence probe observes duplicate datagrams.
- `chaos.activation` — clean sequence-numbered UDP control exchange is observed.
