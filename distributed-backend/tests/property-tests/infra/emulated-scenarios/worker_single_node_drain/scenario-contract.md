# Scenario contract: `worker_single_node_drain`
...
- External dependencies: settlement-worker, nsqd, trade-settlement.
- Workload actors/input schema: `identified settlement message plus healthy control message` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_NODE_DRAIN`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_NODE_DRAIN`
- Exact target: `node hosting a selected encore-backend pod that runs settlement-worker`
...
- Affected path/resource: `nsqd-to-worker replicas`
- Activation barrier: selected worker/node and unaffected worker are ready.
- Litmus definition/reference: `node-drain` against `node hosting a selected encore-backend pod that runs settlement-worker` on `nsqd-to-worker replicas`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement, node unschedulable state, pod relocation, and broker state prove the effect.
...
- Release/recovery barrier: replacement topology is ready and scenario-owned messages leave in-flight state.
...
- `recovery.condition` — replacement topology is ready and scenario-owned messages leave in-flight state.
...
- `chaos.target` — Litmus targets exactly node hosting a selected encore-backend pod that runs settlement-worker and the affected path is nsqd-to-worker replicas.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement, node unschedulable state, pod relocation, and broker state prove the effect.
- `chaos.activation` — selected worker/node and unaffected worker are ready.
...
The Dagger Kind topology currently has one control-plane node and the local overlay has one encore-backend replica hosting the worker, so no unaffected worker can remain during a drain.
