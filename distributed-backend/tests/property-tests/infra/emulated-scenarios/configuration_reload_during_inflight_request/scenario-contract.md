# Scenario contract: `configuration_reload_during_inflight_request`
...
- External dependencies: encore-backend, quilkin.
- Workload actors/input schema: `identified pre-reload and post-reload requests` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_CONFIGURATION_RELOAD`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_CONFIGURATION_RELOAD`
- Exact target: `gateway configuration source`

- Affected path/resource: `configuration-source-to-gateway`
- Activation barrier: pre-reload request and old complete configuration version are observed.
...
- Independent effect witness: application configuration-version observation proves one complete version per request.
...
- Release/recovery barrier: new configuration version is active and control request succeeds.
...
- `recovery.condition` — new configuration version is active and control request succeeds.
...
The production gateway does not expose a versioned observable hot-reload control boundary.
