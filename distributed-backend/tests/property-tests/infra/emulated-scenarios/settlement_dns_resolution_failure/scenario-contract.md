# Scenario contract: `settlement_dns_resolution_failure`
...
- External dependencies: settlement-worker, trade-settlement, nsqd.
...
3. `activate-scenario-chaos` — `LITMUS_DNS_ERROR`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_DNS_ERROR`
- Exact target: `settlement-worker DNS lookup for trade-settlement service`
- Runtime-enforced Kubernetes target policy: `{"kind":"deployment","selector":"app.kubernetes.io/name=encore-backend"}`.
- Runtime-enforced fixed Litmus environment: `{"TARGET_HOSTNAMES":"[\"trade-settlement.${RUN_NAMESPACE}.svc.cluster.local\"]"}`.
- Affected path/resource: `worker-to-cluster-dns-to-settlement`
- Activation barrier: worker-origin control lookup resolves the exact service name.
- Litmus definition/reference: `pod-dns-error` against `settlement-worker DNS lookup for trade-settlement service` on `worker-to-cluster-dns-to-settlement`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent worker-origin DNS probe observes the declared failure.
...
- Release/recovery barrier: DNS fault is absent, lookup resolves, and work message reaches terminal handling.
...
- `recovery.condition` — DNS fault is absent, lookup resolves, and work message reaches terminal handling.
...
- `chaos.target` — Litmus targets exactly settlement-worker DNS lookup for trade-settlement service and the affected path is worker-to-cluster-dns-to-settlement.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent worker-origin DNS probe observes the declared failure.
- `chaos.activation` — worker-origin control lookup resolves the exact service name.
