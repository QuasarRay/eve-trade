# Scenario contract: `database_backup_restore_recovery`
...
- External dependencies: postgres, encore-backend, trade-settlement, settlement-worker, nsqd.
- Workload actors/input schema: `restore-boundary status and replay workload` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `EXTERNAL_DATABASE_RESTORE`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `EXTERNAL_DATABASE_RESTORE`
- Exact target: `postgres primary`

- Affected path/resource: `application-to-restored-postgres`
- Activation barrier: backup identity and requested restore point are recorded.
...
- Independent effect witness: independent SQL probe observes the restored timeline and exact recovery point.
...
- Release/recovery barrier: all application pools reconnect and authoritative state is queryable.
...
- `recovery.condition` — all application pools reconnect and authoritative state is queryable.
...
A real point-in-time restore provider/backup artifact is not available in the local Kind topology.
