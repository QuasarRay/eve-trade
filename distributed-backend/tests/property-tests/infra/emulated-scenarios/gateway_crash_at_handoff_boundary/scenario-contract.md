# Scenario contract: `gateway_crash_at_handoff_boundary`
...
- Workload actors/input schema: `authenticated UDP request and same-identity replay` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `gateway container`
...
- Affected path/resource: `quilkin-to-gateway-to-market handoff`
- Activation barrier: production-disabled gateway failpoint confirms the exact named boundary.
- Litmus definition/reference: `container-kill` against `gateway container` on `quilkin-to-gateway-to-market handoff`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Kubernetes observes the old container exit after failpoint confirmation.
...
- Release/recovery barrier: replacement gateway is ready and replay lookup is observable.
...
- `recovery.condition` — replacement gateway is ready and replay lookup is observable.
...
- `chaos.target` — Litmus targets exactly gateway container and the affected path is quilkin-to-gateway-to-market handoff.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Kubernetes observes the old container exit after failpoint confirmation.
- `chaos.activation` — production-disabled gateway failpoint confirms the exact named boundary.
...
The gateway exposes no production-disabled persistence/handoff failpoints for exact boundary synchronization.
