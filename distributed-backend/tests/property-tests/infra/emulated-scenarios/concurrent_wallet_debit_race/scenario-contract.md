# Scenario contract: `concurrent_wallet_debit_race`
...
- Workload actors/input schema: `multiple settlements sharing one wallet` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `wallet row`

- Affected path/resource: `concurrent wallet-debit transactions`
- Activation barrier: all contenders report arrival before the same wallet mutation boundary.
...
- Release/recovery barrier: all contenders finish and wallet row locks are released.
...
The real settlement path has no wallet pre-mutation race barrier.
