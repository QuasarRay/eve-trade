# Scenario contract: `graceful_shutdown_with_inflight_work`
...
- Workload actors/input schema: `queued and in-flight identified request/message` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_GRACEFUL_POD_TERMINATION`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_GRACEFUL_POD_TERMINATION`
- Exact target: `scenario-selected application pod`

- Affected path/resource: `client-or-broker-to-selected-service`
- Activation barrier: request/message identity is observed at the declared queued or in-flight boundary.
...
- Independent effect witness: Kubernetes termination state plus application/broker observations prove drain disposition.
...
- Release/recovery barrier: replacement is ready and unfinished work is either completed or redeliverable.
...
- `recovery.condition` — replacement is ready and unfinished work is either completed or redeliverable.
