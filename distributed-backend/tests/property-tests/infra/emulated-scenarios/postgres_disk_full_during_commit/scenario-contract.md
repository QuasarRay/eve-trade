# Scenario contract: `postgres_disk_full_during_commit`
...
- External dependencies: trade-settlement, postgres.
- Workload actors/input schema: `identified settlement transaction` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_DISK_FILL_AT_TRANSACTION_BARRIER`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_DISK_FILL_AT_TRANSACTION_BARRIER`
- Exact target: `postgres data filesystem`
- Runtime-enforced Kubernetes target policy: `{"kind":"deployment","selector":"app.kubernetes.io/name=postgres"}`.
- Runtime-enforced fixed Litmus environment: `{}`.
- Affected path/resource: `settlement-to-postgres`
- Activation barrier: transaction failpoint confirms the pre-commit phase.
- Litmus definition/reference: `disk-fill` against `postgres data filesystem` on `settlement-to-postgres`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent filesystem probe observe declared utilization.
...
- Release/recovery barrier: fill files are removed, filesystem is writable, and PostgreSQL is ready.
...
- `recovery.condition` — fill files are removed, filesystem is writable, and PostgreSQL is ready.
...
- `chaos.target` — Litmus targets exactly postgres data filesystem and the affected path is settlement-to-postgres.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent filesystem probe observe declared utilization.
- `chaos.activation` — transaction failpoint confirms the pre-commit phase.
...
No safe transaction barrier synchronizes disk fill with the exact PostgreSQL commit window.
