# Scenario contract: `observability_backend_outage`
...
- External dependencies: encore-backend, otel-collector, postgres.
- Workload actors/input schema: `identified business request plus telemetry control event` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_SCALE_OBSERVABILITY_BACKEND_TO_ZERO`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_SCALE_OBSERVABILITY_BACKEND_TO_ZERO`
- Exact target: `optional observability collector/exporter`

- Affected path/resource: `application-to-observability-backend`
- Activation barrier: business and telemetry control paths are healthy before outage.
...
- Independent effect witness: Kubernetes zero-ready state and independent exporter probe prove outage.
...
- Release/recovery barrier: observability backend recovers and application remains ready.
...
- `recovery.condition` — observability backend recovers and application remains ready.
...
The local Dagger deployment does not install an OpenTelemetry collector and its application configuration disables OTLP export, so a real backend outage cannot yet be established.
