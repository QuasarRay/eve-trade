# Scenario contract: `database_connection_failure_window`
...
- External dependencies: trade-settlement, postgres, encore-backend.
- Workload actors/input schema: `identified database-backed request and connection-pool probes` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_NETWORK_PARTITION`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_NETWORK_PARTITION`
- Exact target: `application egress to postgres`
...
- Affected path/resource: `application-to-postgres`
- Activation barrier: clean SQL probe and declared request phase are observed.
- Litmus definition/reference: `pod-network-partition` against `application egress to postgres` on `application-to-postgres`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent application-origin SQL probe failure are required.
...
- Release/recovery barrier: partition is absent, SQL probe succeeds, and pool capacity returns to baseline.
...
- `recovery.condition` — partition is absent, SQL probe succeeds, and pool capacity returns to baseline.
...
- `chaos.target` — Litmus targets exactly application egress to postgres and the affected path is application-to-postgres.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent application-origin SQL probe failure are required.
- `chaos.activation` — clean SQL probe and declared request phase are observed.
