# Scenario contract: `udp_key_rotation_overlap_window`
...
- Workload actors/input schema: `old-key, new-key, and invalid-key authenticated UDP requests` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_CREDENTIAL_ROTATION`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_CREDENTIAL_ROTATION`
- Exact target: `gateway HMAC keyset`

- Affected path/resource: `configuration-source-to-gateway-authenticator`
- Activation barrier: old keyset version and authenticated control request are observed.
...
- Independent effect witness: application keyset-version observation proves overlap and retirement phases.
...
- Release/recovery barrier: new key is active, retired key is outside overlap, and control request succeeds.
...
- `recovery.condition` — new key is active, retired key is outside overlap, and control request succeeds.
...
The gateway does not expose an observable versioned credential reload boundary.
