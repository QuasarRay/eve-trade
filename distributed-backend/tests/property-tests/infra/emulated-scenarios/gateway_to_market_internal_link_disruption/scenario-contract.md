# Scenario contract: `gateway_to_market_internal_link_disruption`
...
- Workload actors/input schema: `identified authenticated gateway request and durable Market status probe` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `APPLICATION_INTERNAL_DEPENDENCY_FAILPOINT`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `APPLICATION_INTERNAL_DEPENDENCY_FAILPOINT`
- Exact target: `Gateway-to-Market call boundary inside encore-backend`

- Affected path/resource: `gateway-to-market in-process call`
- Activation barrier: gateway request reaches the exact pre-Market-call boundary.
...
- Independent effect witness: application boundary observation proves the Market call is unavailable while the process stays live.
...
- Release/recovery barrier: the failpoint is absent and a control Gateway-to-Market request succeeds.
...
- `recovery.condition` — the failpoint is absent and a control Gateway-to-Market request succeeds.
...
Gateway and Market are co-located in the Encore process, so no physical network partition exists; the repository also has no production-disabled boundary failpoint for this call.
