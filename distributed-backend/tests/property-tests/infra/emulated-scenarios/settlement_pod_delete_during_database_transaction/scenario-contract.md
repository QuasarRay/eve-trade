# Scenario contract: `settlement_pod_delete_during_database_transaction`
...
- Workload actors/input schema: `identified multi-operation settlement request` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_POD_DELETE_AT_TRANSACTION_BARRIER`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_POD_DELETE_AT_TRANSACTION_BARRIER`
- Exact target: `trade-settlement pod selected by exact labels and UID`
...
- Activation barrier: production-disabled transaction barrier confirms the transaction is open before commit.
- Litmus definition/reference: `pod-delete` against `trade-settlement pod selected by exact labels and UID` on `settlement-to-postgres`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement, Kubernetes UID loss, and independent pg_stat_activity observation prove overlap.
...
- Release/recovery barrier: replacement settlement pod is ready and transaction/idempotency state is queryable.
...
- `recovery.condition` — replacement settlement pod is ready and transaction/idempotency state is queryable.
...
- `chaos.target` — Litmus targets exactly trade-settlement pod selected by exact labels and UID and the affected path is settlement-to-postgres.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement, Kubernetes UID loss, and independent pg_stat_activity observation prove overlap.
- `chaos.activation` — production-disabled transaction barrier confirms the transaction is open before commit.
...
No safe application transaction barrier exposes the exact open-transaction window.
