# Scenario contract: `postgres_restart_during_settlement`
...
- Workload actors/input schema: `identified settlement request and status recovery` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `postgres pod selected by exact labels and UID`
...
- Activation barrier: request identity and original PostgreSQL pod UID are recorded.
- Litmus definition/reference: `pod-delete` against `postgres pod selected by exact labels and UID` on `settlement-to-postgres`.
...
- Release/recovery barrier: replacement PostgreSQL is ready and status query succeeds.
...
- `recovery.condition` — replacement PostgreSQL is ready and status query succeeds.
...
- `chaos.target` — Litmus targets exactly postgres pod selected by exact labels and UID and the affected path is settlement-to-postgres.
...
- `chaos.activation` — request identity and original PostgreSQL pod UID are recorded.
