# Scenario contract: `rolling_settlement_replica_replacement`
...
- Workload actors/input schema: `identified settlement requests` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `trade-settlement deployment`

- Affected path/resource: `worker-to-settlement-to-postgres`
- Activation barrier: request identity is observed before old replica termination.
...
- Independent effect witness: pod UID transition and SQL transaction/idempotency observations bind the request across rollout.
...
- Release/recovery barrier: rollout completes and settlement status is queryable.
...
- `recovery.condition` — rollout completes and settlement status is queryable.
