# Scenario contract: `udp_receive_buffer_saturation`
...
- Workload actors/input schema: `sequence-numbered authenticated UDP burst above the observed socket drain rate` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_UDP_SOCKET_BUFFER_SATURATION`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_UDP_SOCKET_BUFFER_SATURATION`
- Exact target: `selected gateway UDP socket and host receive queue`

- Affected path/resource: `quilkin-to-gateway-udp-socket`
- Activation barrier: socket identity, receive-buffer capacity, and clean control exchange are observed.
...
- Independent effect witness: independent kernel socket/drop counters and application receipts identify the saturated interval.
...
- Release/recovery barrier: receive queue and drop counters stabilize and a clean control exchange succeeds.
...
- `recovery.condition` — receive queue and drop counters stabilize and a clean control exchange succeeds.
...
The local deployment does not expose a run-scoped kernel socket/drop probe that can bind receive buffer saturation to the selected gateway socket and exact request identities.
