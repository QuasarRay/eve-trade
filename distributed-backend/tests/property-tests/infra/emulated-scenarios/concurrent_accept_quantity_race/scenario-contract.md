# Scenario contract: `concurrent_accept_quantity_race`
...
- Workload actors/input schema: `multiple accept requests for one exact trade version` with exact request, actor, entity, scenario, seed, and action identities.
...
- Exact target: `trade and item-escrow rows`

- Affected path/resource: `concurrent accept transactions`
- Activation barrier: all contenders report the same pre-mutation trade version/quantity boundary.
...
- Release/recovery barrier: all contenders finish and row locks are released.
...
- `recovery.condition` — all contenders finish and row locks are released.
...
The real settlement path has no pre-mutation race barrier.
