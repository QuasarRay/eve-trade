# Scenario contract: `worker_to_settlement_directional_partition`
...
- Workload actors/input schema: `identified settlement work message` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_DIRECTIONAL_NETWORK_PARTITION`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_DIRECTIONAL_NETWORK_PARTITION`
- Exact target: `encore-backend pod egress from settlement-worker to trade-settlement`
...
- Affected path/resource: `worker-to-trade-settlement`
- Activation barrier: worker observes the identified work message before activation.
- Litmus definition/reference: `pod-network-partition` against `encore-backend pod egress from settlement-worker to trade-settlement` on `worker-to-trade-settlement`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent worker-origin gRPC reachability probe failure are required.
...
- Release/recovery barrier: partition is absent, gRPC probe succeeds, and message reaches terminal handling.
...
- `recovery.condition` — partition is absent, gRPC probe succeeds, and message reaches terminal handling.
...
- `chaos.target` — Litmus targets exactly encore-backend pod egress from settlement-worker to trade-settlement and the affected path is worker-to-trade-settlement.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent worker-origin gRPC reachability probe failure are required.
- `chaos.activation` — worker observes the identified work message before activation.
