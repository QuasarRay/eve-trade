# Scenario contract: `concurrent_outbox_claim_race`
...
- Workload actors/input schema: `one eligible outbox row plus dispatcher contenders` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `APPLICATION_OUTBOX_CLAIM_BARRIER`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `APPLICATION_OUTBOX_CLAIM_BARRIER`
- Exact target: `outbox row or per-trade ordered row set`

- Affected path/resource: `dispatcher-to-postgres claim`
- Activation barrier: all dispatchers report arrival before claim acquisition.
...
- Independent effect witness: independent PostgreSQL lock/activity probe identifies every dispatcher contender.
...
- Release/recovery barrier: dispatchers finish and no scenario-owned claim remains stale.
...
- `recovery.condition` — dispatchers finish and no scenario-owned claim remains stale.
...
The dispatcher has no production-disabled pre-claim barrier.
