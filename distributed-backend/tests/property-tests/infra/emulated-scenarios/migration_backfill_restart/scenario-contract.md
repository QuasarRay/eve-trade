# Scenario contract: `migration_backfill_restart`
...
- Workload actors/input schema: `version-bound pre-migration fixture containing multiple backfill batches` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `establish-controlled-prerequisite` — `DAGGER_RESTART_MIGRATION_AT_BACKFILL_BARRIER`; barrier `controlled-prerequisite-witnessed`.
...
- Action: `DAGGER_RESTART_MIGRATION_AT_BACKFILL_BARRIER`
- Exact target: `settlement database migration job`

- Affected path/resource: `migration-job-to-postgres`
- Activation barrier: the exact durable backfill checkpoint and remaining source rows are independently observed.
...
- Independent effect witness: Kubernetes job identity and SQL checkpoint/output rows prove interruption at that boundary.
...
- Release/recovery barrier: the same migration revision completes and every source row has one required derived row.
...
- `recovery.condition` — the same migration revision completes and every source row has one required derived row.
...
The current migration job executes forward-only SQL files without a restartable batch checkpoint or an observable production-disabled backfill barrier.
