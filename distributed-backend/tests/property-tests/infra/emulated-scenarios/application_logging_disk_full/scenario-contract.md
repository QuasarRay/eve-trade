# Scenario contract: `application_logging_disk_full`
...
- External dependencies: encore-backend, postgres.
- Workload actors/input schema: `identified request with durable status lookup` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_DISK_FILL`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_DISK_FILL`
- Exact target: `encore-backend log filesystem`
...
- Affected path/resource: `application-to-log-sink`
- Activation barrier: application log path and clean filesystem baseline are observed.
- Litmus definition/reference: `disk-fill` against `encore-backend log filesystem` on `application-to-log-sink`.
...
- Release/recovery barrier: fill files are removed and application readiness is restored.
...
- `recovery.condition` — fill files are removed and application readiness is restored.
...
- `chaos.target` — Litmus targets exactly encore-backend log filesystem and the affected path is application-to-log-sink.
...
- `chaos.activation` — application log path and clean filesystem baseline are observed.
