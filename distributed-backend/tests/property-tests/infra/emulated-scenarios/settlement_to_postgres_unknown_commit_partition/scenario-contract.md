# Scenario contract: `settlement_to_postgres_unknown_commit_partition`
...
- External dependencies: trade-settlement, postgres, settlement-worker.
- Workload actors/input schema: `identified settlement request and idempotency status lookup` with exact request, actor, entity, scenario, seed, and action identities.
...
3. `activate-scenario-chaos` — `LITMUS_DIRECTIONAL_NETWORK_PARTITION_AT_TRANSACTION_BARRIER`; barrier `chaos-control-plane-acknowledged`.
...
- Action: `LITMUS_DIRECTIONAL_NETWORK_PARTITION_AT_TRANSACTION_BARRIER`
- Exact target: `trade-settlement egress to postgres`
- Runtime-enforced Kubernetes target policy: `{"kind":"deployment","selector":"app.kubernetes.io/name=trade-settlement"}`.
- Runtime-enforced fixed Litmus environment: `{"NAMESPACE_SELECTOR":"kubernetes.io/metadata.name=${RUN_NAMESPACE}","POD_SELECTOR":"app.kubernetes.io/name=postgres","POLICY_TYPES":"egress"}`.
- Affected path/resource: `settlement-to-postgres`
- Activation barrier: transaction boundary failpoint confirms before-commit or after-commit state.
- Litmus definition/reference: `pod-network-partition` against `trade-settlement egress to postgres` on `settlement-to-postgres`.
- Control-plane acknowledgement: action/scenario/run-bound resource identity and status.
- Independent effect witness: Litmus acknowledgement and independent settlement-origin PostgreSQL probe failure are required.
...
- Release/recovery barrier: partition is absent, SQL probe succeeds, and idempotency status is queryable.
...
- `recovery.condition` — partition is absent, SQL probe succeeds, and idempotency status is queryable.
...
- `chaos.target` — Litmus targets exactly trade-settlement egress to postgres and the affected path is settlement-to-postgres.
- `chaos.control_plane` — A scenario/action-bound Litmus control-plane acknowledgement is required but is not sufficient.
- `chaos.independent_effect` — Litmus acknowledgement and independent settlement-origin PostgreSQL probe failure are required.
- `chaos.activation` — transaction boundary failpoint confirms before-commit or after-commit state.
...
No safe transaction barrier distinguishes before-commit from after-commit acknowledgement loss.
