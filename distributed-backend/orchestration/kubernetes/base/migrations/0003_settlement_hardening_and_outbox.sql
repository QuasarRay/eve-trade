-- Forward-only hardening applied after the immutable 0001 and 0002 migrations.
...
-- Upgrade ledgers created by 0001 before the hardened table definition and
-- projection triggers below are evaluated. The append-only trigger is restored
-- later in this migration after the one-time deterministic backfill.
...
ALTER TABLE item_stack_ledger
    ...
    ADD COLUMN IF NOT EXISTS previous_item_stack_ledger_hash TEXT,
    ADD COLUMN IF NOT EXISTS ledger_payload_hash TEXT,
    ADD COLUMN IF NOT EXISTS item_stack_ledger_hash TEXT,
    ADD COLUMN IF NOT EXISTS stack_state_before TEXT,
    ADD COLUMN IF NOT EXISTS stack_state_after TEXT;

DO $$
DECLARE
    ledger_row RECORD;
    prior_hash TEXT := 'GENESIS';
    current_stack_id UUID;
    inferred_state_before TEXT;
    inferred_state_after TEXT;
    computed_payload_hash TEXT;
    computed_ledger_hash TEXT;
BEGIN
    IF EXISTS (
        SELECT 1
        FROM item_stack_ledger
        WHERE ledger_sequence IS NULL
           OR previous_item_stack_ledger_hash IS NULL
           OR ledger_payload_hash IS NULL
           OR item_stack_ledger_hash IS NULL
           OR stack_state_before IS NULL
           OR stack_state_after IS NULL
    ) THEN
        FOR ledger_row IN
            SELECT *
            FROM item_stack_ledger
            ORDER BY item_stack_id, stack_version_after, created_at, item_stack_ledger_id
        LOOP
            IF current_stack_id IS DISTINCT FROM ledger_row.item_stack_id THEN
                current_stack_id := ledger_row.item_stack_id;
                prior_hash := 'GENESIS';
            END IF;

            inferred_state_before := CASE ledger_row.entry_kind
                WHEN 'CREATE_STACK' THEN 'ABSENT'
                WHEN 'TRANSFER_TO_ESCROW' THEN 'ACTIVE'
                WHEN 'TRANSFER_FROM_ESCROW_TO_NEW_OWNER' THEN 'LOCKED'
                WHEN 'TRANSFER_FROM_ESCROW_TO_PREVIOUS_OWNER' THEN 'LOCKED'
                WHEN 'MERGE_IN' THEN 'ACTIVE'
                WHEN 'MERGE_OUT' THEN 'ACTIVE'
                ELSE 'ACTIVE'
            END;
            inferred_state_after := CASE ledger_row.entry_kind
                WHEN 'TRANSFER_TO_ESCROW' THEN
                    CASE WHEN ledger_row.quantity_after = 0 THEN 'DEPLETED' ELSE 'LOCKED' END
                WHEN 'MERGE_OUT' THEN 'MERGED'
                ELSE CASE WHEN ledger_row.quantity_after = 0 THEN 'DEPLETED' ELSE 'ACTIVE' END
            END;

            computed_payload_hash := compute_item_stack_ledger_payload_hash(
                ledger_row.settlement_step_id,
                ledger_row.item_stack_id,
                ledger_row.stack_version_after,
                ledger_row.item_type_id,
                ledger_row.owner_id,
                ledger_row.station_id,
                ledger_row.entry_kind,
                ledger_row.quantity_delta,
                ledger_row.quantity_before,
                ledger_row.quantity_after,
                inferred_state_before,
                inferred_state_after,
                ledger_row.stack_version_before,
                ledger_row.stack_version_after,
                ledger_row.stack_checksum_before,
                ledger_row.stack_checksum_after
            );
            computed_ledger_hash := compute_item_stack_ledger_hash(prior_hash, computed_payload_hash);

            UPDATE item_stack_ledger
            SET ledger_sequence = ledger_row.stack_version_after,
                previous_item_stack_ledger_hash = prior_hash,
                ledger_payload_hash = computed_payload_hash,
                item_stack_ledger_hash = computed_ledger_hash,
                stack_state_before = inferred_state_before,
                stack_state_after = inferred_state_after
            WHERE item_stack_ledger_id = ledger_row.item_stack_ledger_id;

            prior_hash := computed_ledger_hash;
        END LOOP;
    END IF;
END;
$$;
...
ALTER TABLE item_stack_ledger
    ...
    ALTER COLUMN previous_item_stack_ledger_hash SET NOT NULL,
    ALTER COLUMN ledger_payload_hash SET NOT NULL,
    ALTER COLUMN item_stack_ledger_hash SET NOT NULL,
    ALTER COLUMN stack_state_before SET NOT NULL,
    ALTER COLUMN stack_state_after SET NOT NULL;

ALTER TABLE item_stack_ledger
    DROP CONSTRAINT IF EXISTS item_stack_ledger_sequence_positive_chk,
    DROP CONSTRAINT IF EXISTS item_stack_ledger_state_before_chk,
    DROP CONSTRAINT IF EXISTS item_stack_ledger_state_after_chk,
    DROP CONSTRAINT IF EXISTS item_stack_ledger_sequence_version_chk;

ALTER TABLE item_stack_ledger
    ADD CONSTRAINT item_stack_ledger_sequence_positive_chk CHECK (ledger_sequence > 0),
    ADD CONSTRAINT item_stack_ledger_state_before_chk
        CHECK (stack_state_before IN ('ABSENT', 'ACTIVE', 'LOCKED', 'DEPLETED', 'MERGED')),
    ADD CONSTRAINT item_stack_ledger_state_after_chk
        CHECK (stack_state_after IN ('ACTIVE', 'LOCKED', 'DEPLETED', 'MERGED')),
    ADD CONSTRAINT item_stack_ledger_sequence_version_chk CHECK (ledger_sequence = stack_version_after);
...
ALTER TABLE settlement_operation
    ADD COLUMN IF NOT EXISTS work_payload JSONB,
    ADD COLUMN IF NOT EXISTS work_payload_hash TEXT,
...
CREATE TABLE IF NOT EXISTS settlement_outbox (
    operation_id UUID PRIMARY KEY REFERENCES settlement_operation(operation_id) ON DELETE CASCADE,
    delivery_state TEXT NOT NULL DEFAULT 'PENDING' CHECK (delivery_state IN ('PENDING', 'IN_FLIGHT', 'DELIVERED')),
...
ON settlement_outbox (delivery_state, lease_expires_at, created_at)
